...
export function LessonLobby(){const{bookingId}=useParams();const[params]=useSearchParams();const{search}=useLocation();const navigate=useNavigate();const[cam,setCam]=useState(true);
 ...
 const[mic,setMic]=useState(()=>!params.get('as'));const[testing,setTesting]=useState(false);const[short,setShort]=useState(false);const{user}=useAuth();const{booking:session,payment}=useBookedSession(bookingId);const preview=useCameraPreview(cam);
