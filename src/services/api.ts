export type ApiEnvelope<T>={success:boolean;data:T;message?:string;errors?:unknown};const API_URL=import.meta.env.VITE_API_URL||'/api/v1';
/** Full URL for a path, for requests that cannot go through api(), such as beacons. */
export const apiUrl=(path:string)=>`${API_URL}${path}`;/* A dev proxy answers with its own error page when the API is down, so these are a failed
